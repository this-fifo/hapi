...
export class SessionCache {
    ...
    softArchiveSession(sessionId: string): boolean {
        ...
        if (session.active) {
            throw new Error('Cannot archive active session; stop it first')
        }
    ...
    deleteArchivedOlderThan(namespace: string, archivedDays: number): number {
        ...
        for (const id of ids) {
            ...
            this.pendingThinkingUntilBySessionId.delete(id)
            this.runtimeConfigUpdatedAtBySessionId.delete(id)
