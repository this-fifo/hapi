...
export function useUpdateServerSettings(api: ApiClient | null) {
    ...
    return useMutation({
        mutationFn: async (updates: Partial<ServerSettings>): Promise<ServerSettings> => {
            if (!api) throw new Error('API unavailable')
            return await api.updateServerSettings(updates)
        },
        onSuccess: (data) => {
...
export function useBulkArchive(api: ApiClient | null) {
    const queryClient = useQueryClient()
    return useMutation({
        mutationFn: async (opts: { olderThanDays?: number }): Promise<{ ok: boolean; archived: number }> => {
            if (!api) throw new Error('API unavailable')
            return await api.bulkArchive(opts)
        },
        onSuccess: () => {
