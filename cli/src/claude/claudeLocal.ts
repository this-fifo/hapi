...
export async function claudeLocal(opts: {
    ...
    //
    // The SDK query() used in remote mode sets CLAUDE_CODE_ENTRYPOINT='sdk-ts'
    // on the current process. If leaked into the local spawn, Claude Code
    // thinks it was SDK-launched and excludes the session from
    // `claude --resume`. Destructure it out so the child uses its own
    // default entrypoint.
